...
def get_image(image_id):
    ...
    img_all = np.zeros((img_RGB2_resize.shape[0],img_RGB2_resize.shape[1],20), 'uint8')
    img_all[..., 0:3] = img_RGB2_resize
    img_all[..., 3] = img_P2_resize
    img_all[..., 4:12] = img_M2_resize
    img_all[..., 12:21] = img_A2_resize
...
def stick_all_train(class_id):
    print("let's stick all imgs together")
    #s = 835
    s = 512

    #x = np.zeros((5 * s, 5 * s, 8))
    x = np.zeros((5 * s, 5 * s, 20))
    y = np.zeros((5 * s, 5 * s))
    ...
    for i in range(5):
        for j in range(5):
            ...
            y[s * i:s * i + s, s * j:s * j + s] = generate_mask_for_image_and_class(
                (img.shape[0], img.shape[1]), id, class_id)[:s, :s]

    print(np.amax(y), np.amin(y))

    np.save('data/x_trn_' + str(N_Cls) + '_' + slug, x)
    np.save('data/y_trn_' + str(class_id) + '_' + slug, y)
    #np.save('data/x_trn_%d' % N_Cls, x)
    #np.save('data/y_trn_%d' % N_Cls, y)


def get_patches(img, msk, class_id, amt=10000, aug=True):
    is2 = int(1.0 * ISZ) # ISZ is the patch size
    xm, ym = img.shape[0] - is2, img.shape[1] - is2 # 835 - 160?

    x, y = [], []

    tr = [0.4, 0.1, 0.1, 0.15, 0.3, 0.95, 0.1, 0.05, 0.001, 0.005] # Thresholds to determine whether we should train the class
    for i in range(amt):
...
def make_val(class_id):
    print("let's pick some samples for validation")
    img = np.load('data/x_trn_' + str(N_Cls) + '_' + slug + '.npy')
    msk = np.load('data/y_trn_' + str(class_id) + '_' + slug + '.npy')
    x, y = get_patches(img, msk, class_id, amt=3000)
    #x, y = get_patches(img, msk, class_id, amt=100)

    #np.save('data/x_tmp_%d' % N_Cls, x)
    #np.save('data/y_tmp_%d' % N_Cls, y)
    np.save('data/x_tmp_' + str(N_Cls) + '_' + slug, x)
    np.save('data/y_tmp_' + str(class_id) + '_' + slug, y)
...
def get_unet():
    ...
    model.compile(optimizer=Adam(), loss=jaccard_coef, metrics=[jaccard_coef, jaccard_coef_int, 'accuracy'])
    return model


def calc_jacc(model, class_id):
    #img = np.load('data/x_tmp_%d.npy' % N_Cls)
    #msk = np.load('data/y_tmp_%d.npy' % N_Cls)
    img = np.load('data/x_tmp_' + str(N_Cls) + '_' + slug + '.npy')
    msk = np.load('data/y_tmp_' + str(class_id) + '_' + slug + '.npy')
    ...
    #t_msk = msk
    #t_prd = prd
    t_msk = msk.reshape(msk.shape[0] * msk.shape[2], msk.shape[3])
    t_prd = prd.reshape(msk.shape[0] * msk.shape[2], msk.shape[3])
...
def train_net(class_id):
    print("start train net")
    #x_val, y_val = np.load('data/x_tmp_%d.npy' % N_Cls), np.load('data/y_tmp_%d.npy' % N_Cls)
    x_val = np.load('data/x_tmp_' + str(N_Cls) + '_' + slug + '.npy')
    y_val = np.load('data/y_tmp_' + str(class_id) + '_' + slug + '.npy')
    ...
    #x_trn, y_trn = get_patches(img, msk, class_id)
    #x_trn, y_trn = get_patches(img, msk, class_id, amt = 3000)
    x_trn, y_trn = get_patches(img, msk, class_id, amt = 3000)
    ...
    #model.load_weights('weights/unet_' + str(class_id) + '_' + slug)
    print("Model Checkpoint")
    model_checkpoint = ModelCheckpoint('weights/unet_tmp_' + slug + '.hdf5', monitor='loss', save_best_only=True)
    for i in range(1):
        print("Model fit")
        print(x_trn.shape)
        print(y_trn.shape)
        ...
        #x_trn, y_trn = get_patches(img, msk, class_id)
        #x_trn, y_trn = get_patches(img, msk, class_id, amt = 3000)
        #score, trs = calc_jacc(model, class_id)
        #print('val jk', score)
        #model.save_weights('weights/unet_10_jk%.4f' % score)
        model.save_weights('weights/unet_' + str(class_id) + '_' + slug)

    return model


def predict_id(id, model, tr, class_id):
    #img = M(id)
    img = get_image(id)
    x = stretch_n(img)
    ...
    cnv = np.zeros((512, 512, 20)).astype(np.float32)
    prd = np.zeros((512, 512)).astype(np.float32)
...
def predict_test(model, tr, class_id):
    print("predict test")
    for i, id in enumerate(sorted(set(SB['ImageId'].tolist()))):
        msk = predict_id(id, model, tr, class_id)
        np.save('msk/' + str(class_id) + '_%s' % id, msk)
        if i % 100 == 0: print(i, id)


def make_submit(class_id):
    ...
    for idx, row in df.iterrows():
        ...
        try:
            msk = np.load('msk/' + str(class_id) + '_%s' % id)[kls]
    ...
    print(df.head())
    df.to_csv('subm/' + str(class_id) + slug + '.csv', index=False)


def check_predict(id='6120_2_3'):
    model = get_unet()
    #model.load_weights('weights/unet_10_jk0.7878')
    model.load_weights('weights/unet_10_jk0.6911')
...
if __name__ == '__main__':
    class_id = 1
    stick_all_train(class_id)
    make_val(class_id)
    model = train_net(class_id)
    score, tr = calc_jacc(model, class_id)
    predict_test(model, tr, class_id)
    make_submit(class_id)
