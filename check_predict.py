...
def train_net():
    ...
    for i in range(1):
        print("Model fit")
        model.fit(x_trn, y_trn, batch_size=4, nb_epoch=1, verbose=1, shuffle=True,
...
def check_predict(id='6020_1_1'):
    ...
    plt.figure()
    ax1 = plt.subplot(131)
    ax1.set_title('image ID:6020_1_1')

    r = img[:,:,4]
    g = img[:,:,2]
    b = img[:,:,1]
    rgb = stretch_n(np.dstack((r,g,b)))
