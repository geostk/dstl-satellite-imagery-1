...
def stick_all_train():
    ...
    np.save('data/y_trn_' + str(N_Cls) + '_' + slug, y)
...
def make_val():
    ...
    msk = np.load('data/y_trn_' + str(N_Cls) + '_' + slug + '.npy')
    #x, y = get_patches(img, msk, amt=3000)
    x, y = get_patches(img, msk, amt=10)

    #np.save('data/x_tmp_%d' % N_Cls, x)
    #np.save('data/y_tmp_%d' % N_Cls, y)
    np.save('data/x_tmp_' + str(N_Cls) + '_' + slug, x)
    np.save('data/y_tmp_' + str(N_Cls) + '_' + slug, y)
...
def calc_jacc(model):
    ...
    msk = np.load('data/y_tmp_' + str(N_Cls) + '_' + slug + '.npy')
...
def train_net():
    ...
    y_val = np.load('data/y_tmp_' + str(N_Cls) + '_' + slug + '.npy')
    ...
    x_trn, y_trn = get_patches(img, msk, amt = 10)
    ...
    for i in range(1):
        ...
        model.save_weights('weights/unet_' + slug)
...
def predict_id(id, model, trs):
    ...
    prd = np.zeros((N_Cls, 512, 512)).astype(np.float32)
...
def make_submit():
    ...
    print(df.head())
    df.to_csv('subm/' + slug + '.csv', index=False)
